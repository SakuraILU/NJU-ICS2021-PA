...
def_EHelper(auipc) {
  rtl_addi(s, id_dest->preg, &cpu.pc, c_sext(id_src1->imm,0) );
}
...
def_EHelper(div) {
  if(!*(id_src2->preg)) *(id_dest->preg) = -1;
  else rtl_divs_q(s, id_dest->preg, id_src1->preg, id_src2->preg );
}
...
def_EHelper(divu) {
  ...
  if(!*(id_src2->preg)) *(id_dest->preg) = 0xffffffff;
  else rtl_divu_q(s, id_dest->preg, id_src1->preg, id_src2->preg );
